...
void loop()
{
for (pos =200; pos >= 199; pos -= 1) { // goes from 0 degrees to 180 degrees
...
if (sensor.timeoutOccurred()) 
  ...
  Serial.println();
  if(sensor.readRangeContinuousMillimeters() > 700 && stop_state == 0)  //측정된 거리가 설정 값보다 크고 (위험요소가 측정 되었고) stop_state가 0 일때 (아직 제동하지 않은 상태) 제동
 ...
 if(sensor.readRangeContinuousMillimeters() < 700)   //측정된 거리가 설정 값보다 작다면 (위험요소가 측정 되지 않는다면) stop_state에 0을 넣음
 {
  stop_state = 0;
  }
