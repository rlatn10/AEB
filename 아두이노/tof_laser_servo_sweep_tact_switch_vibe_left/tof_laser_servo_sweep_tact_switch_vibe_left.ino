...
if (sensor.timeoutOccurred()) 
  ...
  Serial.println();
  if(sensor.readRangeContinuousMillimeters() > 1000 && stop_state == 0)  //측정된 거리가 설정 값보다 크고 (위험요소가 측정 되었고) stop_state가 0 일때 (아직 제동하지 않은 상태) 제동
    
   {
    digitalWrite(8, HIGH);                   // 8번 핀에 HIGH신호 (5V)를 출력한다 = 진동모터를 구동
    
    for (pos = 0; pos <= 70; pos += 1)       //0도에서 90도까지 서보모터를 돌려 브레이크 패드를 접촉시켜 휠체어를 제동
       {                                  
        myservo.write(pos);               
    ...
    stop_state = 1;                                //이미 멈추었으니 stop_state를 1로 변경    }
 if(sensor.readRangeContinuousMillimeters() < 1000)   //측정된 거리가 설정 값보다 작다면 (위험요소가 측정 되지 않는다면) stop_state에 0을 넣음
 {
  stop_state = 0;
  }
 if(digitalRead(sw1) == LOW)                 //스위치가 눌리면
 {
  for (pos = 0; pos <= 70; pos += 1)        //0도에서 90도까지 서보모터를 돌려 브레이크 패드를 접촉시켜 휠체어를 제동
       { 
        ...
        delay(1500);                         //모터를 당긴 상태로 1초간 멈추어 안전성을 늘림
       for (pos = 70; pos >= 0; pos -= 1) 
       { 
       myservo.write(pos);              
       delay(10);                              //브레이크를 풀어주는 시간 역시 신속해야 해서 속도를 빠르게 설정
