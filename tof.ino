...
void loop()
{
  ...
  Serial.println();
  if(sensor.readRangeContinuousMillimeters() > 700 && stop_state == 0)  //측정된 거리가 설정 값보다 크고 (위험요소가 측정 되었고) stop_state가 0 일때 (아직 제동하지 않은 상태) 제동한다
    {
      digitalWrite(8, HIGH);         // 8번 핀에 HIGH신호 (5V)를 출력한다 = 진동모터를 구동함
       for (pos = 0; pos <= 90; pos += 1) 
       {//0도에서 90도까지 서보모터를 돌려 휠체어를 제동함
        myservo.write(pos);              // tell servo to go to position in variable 'pos'
        delay(3);                       //브레이크를 잡아주는 시간은 빠를수록 좋다고 생각해서 속도를 더 빠르게 함
       }
       ...
       myservo.write(pos);              // tell servo to go to position in variable 'pos'
       delay(3);                       //브레이크를 풀어주는 시간 역시 빠르면 좋다고 생각해서 속도를 빠르게 해줌.
       }
    stop_state = 1;                   //이미 멈추었으니 stop_state를 1로 바꿔줌 
    }
 if(sensor.readRangeContinuousMillimeters() < 700)   //측정된 거리가 설정 값보다 작다면 (위험요소가 측정 되지 않는다면) stop_state에 0을 넣어줌
 {
  stop_state = 0;
  }
